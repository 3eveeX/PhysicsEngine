...
*
*   Copyright (c) 2022 Xadia Hatch. All Rights Reserved.
*
...
GuiPhysicsState InitGuiPhysics(void)
{
    ...
    state.BodySizeValue = 0.2f;
    state.BodyGravityValue = 10.0f;
    state.BodyRestitutionValue = 1.0f;
    state.BodyDampingValue = 2.0f;
...
void GuiPhysics(GuiPhysicsState *state)
{
    ...
    if (state->PhysicsPanelActive)
    {
        ...
        GuiSliderBar(Rectangle { state->anchor02.x + 80, state->anchor02.y + 88, 184, 16 }, "SIZE", GUI_TEXT(state-> BodySizeValue), & state->BodySizeValue, 0, 5);
        ...
        GuiSlider(Rectangle { state->anchor02.x + 80, state->anchor02.y + 472, 184, 16 }, "FORCE", GUI_TEXT(state->EffectorForceValue), & state->EffectorForceValue, -30000, 30000);
